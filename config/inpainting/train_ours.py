...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            ...
            os.system("rm ./log_inpainting")
            os.symlink(os.path.join(opt["path"]["experiments_root"], ".."), "./log_inpainting")
        ...
        if opt["use_tb_logger"] and "debug" not in opt["name"]:
            ...
            tb_logger = SummaryWriter(log_dir="log_inpainting/{}/tb_logger/".format(opt["name"]))
    else:
        util.setup_logger(
